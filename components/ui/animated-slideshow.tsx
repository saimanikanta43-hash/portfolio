...
interface TextStaggerHoverProps { text: string; index: number }
interface HoverSliderImageProps { index: number; imageUrl: string }
...
function useHoverSliderContext() {
  ...
  if (!context) throw new Error("Must be used within HoverSliderProvider")
  return context
}
      ...
      {characters.map((char, i) => (
        <span key={i} className="relative inline-block overflow-hidden">
          <MotionConfig transition={{
            delay: i * 0.025, duration: 0.3,
            ease: [0.25, 0.46, 0.45, 0.94]
          }}>
            <motion.span className="inline-block opacity-30"
